...
class Cloud extends Phaser.GameObjects.Sprite {
  constructor(scene, x, y) {
    ...
    const textureKey = 'cloudTexture';
    super(scene, x, finalY, textureKey);
...
class Obstacle extends Phaser.GameObjects.Sprite {
  ...
  init() {
    ...
    const tomatoAnimation = this.scene.anims.create({
      key: "tomato",
      frames: this.scene.anims.generateFrameNumbers("tomato", {
        start: 0,
        end: 2,
      }),
      frameRate: 3,
    });
    this.play({ key: "tomato", repeat: -1 });
  }
}
